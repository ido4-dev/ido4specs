...
| **2** | Pending | `ido4specs` plugin creation (skills, agents, hooks, bundled validator copy) |
...
Add an entry to `cross-repo-connections.md` under "Outbound dispatches and writes":

- **Source:** `ido4` (publishing `@ido4/tech-spec-format`)
- **Target:** `ido4specs`
- **Mechanism:** `repository_dispatch` from `ido4`'s `publish.yml` → `ido4specs`'s `update-tech-spec-validator.yml`
- **Payload:** version string
- **Downstream action:** auto-update PR with new bundle, gated on patch/minor only; major bumps require manual review

Mirrors the existing `@ido4/spec-format` → `ido4shape` flow. Same PAT pattern (`IDO4SPECS_DISPATCH_TOKEN`).
