...
| **5** | Pending | Release coordination: publish four npm packages including `@ido4/tech-spec-format`, create `ido4-dev/ido4specs` GitHub repo + `IDO4SPECS_DISPATCH_TOKEN` secret, wire BOTH dispatch steps in `ido4/.github/workflows/publish.yml` (tech-spec-format-published AND spec-format-published → ido4specs), ship marketplace entry, activate `sync-marketplace.yml`, release slimmed `ido4dev` v0.8.0. See `ido4-suite/PLAN.md` Phase 9.5 for the per-sub-step plan (9.5.1–9.5.7). |
...
1. A user can install `ido4specs` alone, run `/ido4specs:create-spec path/to/strategic-spec.md`, and produce a validated technical spec `.md` on disk. No MCP server installed. No `@ido4/mcp` runtime dependency.
2. A user can install `ido4dev` alone (without `ido4specs`) and still do everything `ido4dev` currently does *except* authoring technical specs from scratch. Existing technical specs on disk can still be ingested.
3. A user who installs both plugins sees a smooth handoff: `ido4specs:create-spec` produces a file, the cross-sell footer points at `/ido4dev:ingest-spec`, the next command creates issues.
4. `@ido4/tech-spec-format` is an independent npm package that any third party could consume to validate technical specs without installing anything else.
5. The technical-spec format has an explicit version contract with fail-fast on major mismatch, preventing a repeat of the wildcard-dep silent divergence bug.
6. `bash ~/dev-projects/ido4-suite/scripts/audit-suite.sh` passes clean with `ido4specs` listed as a Tier 1 canonical-pattern plugin.
7. Both `ido4shape`'s and `ido4specs`'s skill namespaces are symmetric — same verbs, different plugin prefix, different domain.
8. The `ido4` monorepo has cleaner internal layers: `@ido4/spec-format` owns strategic parsing, `@ido4/tech-spec-format` owns technical parsing, `@ido4/core` owns the mapper, service, and governance, `@ido4/mcp` owns the MCP surface.
